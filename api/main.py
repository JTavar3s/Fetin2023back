...
#JWT:  JWT é gerado pelo servidor e enviado de volta ao cliente
# Configurando o JWT
...
#Faz todo o Upload das fotos para treinamento
...
class User:
    ...
    def from_dict(data):
        return User(
            data.get("id"), #A partir do ID será gerado um token de validação /me
            data.get("name"),
            data.get("email"),
            data.get("password"),
...
class Photo:
    ...
    def from_dict(data):
        return Photo(
            data.get("id"), # Ao fazer a validação da foto será retornado um ID, o Caminho do arquivo, e o id do usuario relacionado a foto
            data.get("path"),
            data.get("user_id")
        )
    
    def save(self):
        db_photos.add(self.to_dict()) #refere-se à instância que está sendo criada e é usado para atribuir valores aos atributos da instância 
...
def validar_rosto(diretorio_treinamento, foto):
    ...
    print(f'OS TREINAMENTO: {ids_treinamento}')
